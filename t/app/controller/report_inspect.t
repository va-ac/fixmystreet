...
FixMyStreet::override_config {
    ...
    subtest "marking a report as a duplicate automatically adds an update" => sub {
        my $old_state = $report->state;
        $report->comments->delete_all;

        $mech->get_ok("/report/$report_id");
        $mech->submit_form_ok({ button => 'save', with_fields => { state => 'Duplicate', duplicate_of => $report2->id } });
        $report->discard_changes;
        ...
        $mech->content_contains("Thank you for your report. This problem has already been reported.");
